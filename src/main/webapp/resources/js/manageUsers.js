var validateIcon = function(cell, formatterParams){ 
	if (cell.getRow().getData(0)[6]==0) {
		 ...
		 return'<button id='+id+' class=" open_modal btn btn-success btn-sm" data-toggle="modal" data-target="#validate_user_modal" data-id='+id+' data-toggle="tooltip" title="Validater User"><i class="fa fa-check fa-sm" style="color:white"></i> Valider</button>'
...
var deleteUser = function(cell, formatterParams){ 
	    var id=cell.getRow().getData(0)[0].toString();		
		return '<button id='+id+' class=" open_modal btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_user_modal" data-id='+id+' data-toggle="tooltip" title="Delete User"><i class="fa fa-trash fa-sm" style="color:white"></i></button>'

};

var editUser = function(cell, formatterParams){ 
    var id=cell.getRow().getData(0)[0].toString();		
	return '<button id='+id+' class=" open_modal btn btn-info btn-sm" data-toggle="modal" data-target="#edit_user_modal" data-id='+id+' data-toggle="tooltip" title="Editer User"><i class="fa fa-edit fa-sm" style="color:white"></i></button>'
...
//		},
		
		{
			formatter:editUser, align:"center",width:51,headerSort: false, cellClick:function(e, cell){
				...
				var username_id=cell.getRow().getData(0)[5].toString();
				var userType_id=cell.getRow().getData(0)[6].toString();
	           ...
	           // $(".modal-body #password_id").val(prenom_id);
...
$("#validateForm").submit(function (e) {
	e.preventDefault(); // avoid to execute the actual submit of the form to the server and use Ajax instead.
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.success) {
				...
				$("#"+data.id).replaceWith( '<img src="resources/images/Check_24x24.png" alt="IMG">' );	
